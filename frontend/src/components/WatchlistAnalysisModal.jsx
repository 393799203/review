...
import { Modal, Card, Tag, Spin, Rate, Empty, Button } from 'antd';
import { RobotOutlined, ReloadOutlined } from '@ant-design/icons';
import api from '../services/api';

const WatchlistAnalysisModal = ({ visible, stockCode, stockName, onClose }) => {
      ...
      title={
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingRight: 30 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: isMobile ? 14 : 16 }}>股票智能分析</span>
          </div>
          <Button 
            type="text" 
            icon={<ReloadOutlined />} 
    ...
    >
      {loading ? (
        <div style={{ textAlign: 'center', padding: '40px 0' }}>
          <Spin size="large" description="正在分析股票..." />
        </div>
      ) : !analysisData ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8, marginBottom: 8 }}>
                  ...
                  {analysisData.trading_advice.stop_loss_price && (
                    <div>
                      ...
                      <Tag color="red" style={{ fontSize: 11, padding: '2px 6px', margin: 0 }}>
                        {analysisData.trading_advice.stop_loss_price}
                      </Tag>
                    </div>
                  )}
                  ...
                  {analysisData.trading_advice.take_profit_price && (
                    <div>
                      ...
                      <Tag color="green" style={{ fontSize: 11, padding: '2px 6px', margin: 0 }}>
                        {analysisData.trading_advice.take_profit_price}
                      </Tag>
                    </div>
                  )}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8, marginBottom: 8 }}>
                  ...
                  {analysisData.trading_advice.holding_period && (
                    <div>
                      ...
                      <Tag color="cyan" style={{ fontSize: 11, padding: '2px 6px', margin: 0 }}>
                        {analysisData.trading_advice.holding_period}
                      </Tag>
                    </div>
                  )}
            ...
            >
              <div>
                ...
                {analysisData.holding_advice.risk_warning && (
                  <div style={{ marginBottom: 8 }}>
                    <div style={{ fontSize: 11, color: '#666', marginBottom: 2 }}>风险提示</div>
                    <div style={{ fontSize: 12, lineHeight: 1.5, color: '#ff4d4f' }}>
                      {analysisData.holding_advice.risk_warning}
                    </div>
                  </div>
                )}
...
export default WatchlistAnalysisModal;
